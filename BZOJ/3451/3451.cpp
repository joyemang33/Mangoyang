...
namespace FFT{
	...
	inline void DFT(Node *A,  int type){
		...
		for(int k = 2; k <= len; k <<= 1){
			...
			for(int i = 0; i < len; i += k){
				Node now = (Node){1.0, 0.0};
				for(int j = i; j < i + (k >> 1); j++){
					Node x = A[j], y = now * A[j+(k>>1)];
	...
	inline int COV(Node *A, int slen){
...
namespace DivTree{
	...
	inline int calc(Node *A, int u, int d){
		int len = getdis(A, u, 0, d); 
		return FFT::COV(A, len + 2);
	}
...
int main(){
	...
	DivTree::realmain();
	double ans = 0;
	for(int i = 0; i <= n; i++) Ans[i] = (int)(Ans[i] + 0.5);
	for(int i = 0; i <= n; i++) ans += Ans[i] / (double) (i + 1);
	printf("%.4Lf", ans);
	return 0;
}
